...
export function usePostUseCases() {
  ...
  const createPost = useCallback(
    (data: {
      ...
      const useCase = container.getUseCase<CreatePostUseCase>('CreatePostUseCase');
  ...
  const getAllCategories = useCallback(() => {
    const useCase = container.getUseCase<GetAllCategoriesUseCase>('GetAllCategoriesUseCase');
    return useCase.execute();
  }, []);

  const getCategoryBySlug = useCallback((slug: string) => {
    const useCase = container.getUseCase<GetCategoryBySlugUseCase>('GetCategoryBySlugUseCase');
  ...
  const getPostsByCategory = useCallback((categorySlug: string, params: { page?: number; size?: number; sort?: string }) => {
    const useCase = container.getUseCase<GetPostsByCategoryUseCase>('GetPostsByCategoryUseCase');
    return useCase.execute(categorySlug, params);
  }, []);

  const searchPosts = useCallback((keyword: string, params: { page?: number; size?: number; sort?: string }) => {
    const useCase = container.getUseCase<SearchPostsUseCase>('SearchPostsUseCase');
  ...
  const getPostById = useCallback((id: number) => {
    const useCase = container.getUseCase<GetPostByIdUseCase>('GetPostByIdUseCase');
    return useCase.execute(id);
  }, []);

  const createPostNew = useCallback((data: PostCreateDto) => {
    const useCase = container.getUseCase<CreatePostNewUseCase>('CreatePostNewUseCase');
    return useCase.execute(data);
  }, []);

  const updatePost = useCallback((id: number, data: PostCreateDto) => {
    const useCase = container.getUseCase<UpdatePostUseCase>('UpdatePostUseCase');
    return useCase.execute(id, data);
  }, []);

  const deletePost = useCallback((id: number) => {
    const useCase = container.getUseCase<DeletePostUseCase>('DeletePostUseCase');
    return useCase.execute(id);
  }, []);

  const getRepliesByPost = useCallback((postId: number) => {
    const useCase = container.getUseCase<GetRepliesByPostUseCase>('GetRepliesByPostUseCase');
  ...
  const deleteReply = useCallback((replyId: number) => {
    const useCase = container.getUseCase<DeleteReplyUseCase>('DeleteReplyUseCase');
